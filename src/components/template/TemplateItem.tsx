...
import { ActionIcon, Avatar, Badge, Box, Button, Divider, Group, Stack, Text } from "@mantine/core";
import { IconCategory, IconClock, IconEdit, IconLetterCase, IconLetterT, IconMinus, IconPencil, IconPlus, IconStar, IconTag, IconTrash } from "@tabler/icons-react";
    ...
    <Box className={classes.root}>
      ...
      <Stack gap={4} display={collapsed ? 'none' : 'flex'} py={6}>
        ...
        <Group gap={6} align="center">
          {!confirmDelete ?
            <>
              <ActionIcon size='sm' variant="light" color='red' loading={isPending} onClick={() => setConfirmDelete(true)}>
                <IconTrash size={16} color="currentColor" />
              </ActionIcon>
            </>
            :
            <Group gap={4}>
              <Button variant="transparent" size="xs" color="red" p={0} fw={500} onClick={handleDelete}>Yes</Button>
              <Button variant="transparent" size="xs" p={0} fw={500} onClick={() => setConfirmDelete(false)}>No</Button>
            </Group>
          }
        </Group>
      </Stack>
