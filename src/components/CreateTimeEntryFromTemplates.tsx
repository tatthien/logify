...
import { CreateClockifyTimeEntryPayload, createClockifyTimeEntry } from "@/services/clockify/time-entry";
...
export function CreateTimeEntryFromTemplates({ date }: { date: Date }) {
  ...
  return (
    <Menu>
      ...
      <Menu.Dropdown>
        {!templates?.length && <Menu.Item>
          <Stack align="center" gap={6}>
            <Text fz='sm' c='gray.5'>No templates found</Text>
            <Button component={Link} prefetch={true} href="/settings/templates" color="pink" size="xs" fullWidth leftSection={<IconPlus size={16} />}>Create new</Button>
          </Stack>
        </Menu.Item>}
        {templates?.map((template) => (
          <Menu.Item key={template.id} onClick={() => handleCreateTimeEntry(template)}>{template.name}</Menu.Item>
        ))}
      </Menu.Dropdown>
    </Menu>
  )
