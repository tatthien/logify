...
export function MisaScheduleCalendar({ value, disabled = false, onChange }: MisaScheduleCalendarProps) {
  ...
  return (
    <Box className={classes.calendarWrapper}>
      <Flex mb={16} align="center" justify="space-between">
        ...
        <Button variant="transparent" size="compact-md" fz={'xs'} fw={500} onClick={toggleSelect}>
      ...
      <Checkbox.Group className="schedule-calendar" value={localValue} onChange={(value) => handleChange(value)}>
        {dates.map((d, i) => (
          <Checkbox.Card className={classes.checkboxCardRoot} key={i} value={dayjs(d).format('YYYY-MM-DD')}>
            <Text fz="xs" fw={500}>
