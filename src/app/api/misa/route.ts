...
export async function POST(req: NextRequest) {
  ...
  if (!sessionId) {
    return NextResponse.json({ message: 'Mission session ID' }, { status: 400 })
  }
  ...
  if (hours < 9 || hours > 18) {
    return NextResponse.json({ message: 'Not in working hours. Working hours: 09:00 - 18:00' }, { status: 400 })
  }
  ...
  try {
    const res = await fetch('https://amisapp.misa.vn/APIS/TimekeeperAPI/api/TimekeepingRemote/timekeeping-now', {
      method: 'POST',
      credentials: 'include',
      headers: {
